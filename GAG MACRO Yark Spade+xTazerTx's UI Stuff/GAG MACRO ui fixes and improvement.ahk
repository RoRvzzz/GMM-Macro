...
Gui, Add, Edit, x369 y132 w36 h18 vManualBearCraftLock gUpdateCraftLock -Theme cBlack, %ManualBearCraftLock%
    ...
    Gui, Add, Text, x390 y80  w100 h16 gDisplayWebhookValidity BackgroundTrans 0x4
    Gui, Add, Text, x390 y108 w100 h16 gUpdateUserID          BackgroundTrans 0x4
    Gui, Add, Text, x390 y135 w100 h16 gDisplayServerValidity BackgroundTrans 0x4
    Gui, Add, Text, x217 y158 w79  h18 gClearSaves            BackgroundTrans 0x4 
    ...
    Gui, Add, Text, x414 y240 w84  h20 gGui4                  BackgroundTrans 0x4 ;egg
    Gui, Add, Text, x414 y204 w84  h20 gGui3                  BackgroundTrans 0x4 ;pets
    ...
    Gui, Add, Picture, x0 y-6 w520 h425 BackgroundTrans, % mainDir "rbx macro setting  tab Save Webhook button.PNG"
    Gui, Add, Picture, x0 y-2 w520 h425 BackgroundTrans, % mainDir "rbx macro setting  tab Save PS Link button.PNG"
    Gui, Add, Picture, x0 y-5 w520 h425 BackgroundTrans, % mainDir "rbx macro setting  tab Save User ID button.PNG"
   
    Gui, Add, Picture, x0 y-11 w520 h425 BackgroundTrans, % mainDir "rbx macro setting  tab Eggs button1.PNG"
    Gui, Add, Picture, x0 y-14 w520 h425 BackgroundTrans, % mainDir "rbx macro setting  tab Pets button.PNG" 
    Gui, Add, Picture, x0 y-6 w520 h425 BackgroundTrans, % mainDir "rbx macro setting  tab clear save button.PNG"
    ...
    Gui, Add, Checkbox, % "x140 y234 w12 h12 vPingSelected gUpdateSettingColor " . pingColor . (PingSelected ? " Checked" : ""), Discord Item Pings
    
    IniRead, AutoAlign, %settingsFile%, Main, AutoAlign, 0
    autoColor := AutoAlign ? "c90EE90" : "cD3D3D3"
    Gui, Add, Checkbox, % "x105 y259 w12 h12 vAutoAlign gUpdateSettingColor " . autoColor . (AutoAlign ? " Checked" : ""), Auto-Align


    IniRead, MultiInstanceMode, %settingsFile%, Main, MultiInstanceMode, 0
    multiInstanceColor := MultiInstanceMode ? "c90EE90" : "cD3D3D3"
    Gui, Add, Checkbox, % "x160 y282 w12 h12 vMultiInstanceMode gUpdateSettingColor " . multiInstanceColor . (MultiInstanceMode ? " Checked" : ""), Multi-Instance Mode

    IniRead, UINavigationFix, %settingsFile%, Main, UINavigationFix, 0
    uiNavigationFixColor := UINavigationFix ? "c90EE90" : "cD3D3D3"
    Gui, Add, Checkbox, % "x140 y306 w12 h12 vUINavigationFix gUpdateSettingColor " . uiNavigationFixColor . (UINavigationFix ? " Checked" : ""), UI Navigation Fix
    ...
    Gui, Add, Edit, x125 y83 w255 h15 vwebhookURL hwndhWebhookURL +E0x200 +E0x800 -Theme cBlack, %savedWebhook%
    ...
    Gui, Add, Edit, x135 y108 w245 h15 vdiscordUserID +E0x200 +E0x800 -Theme cBlack Background2C2F33, %savedUserID%
    IniRead, savedUserID, %settingsFile%, Main, DiscordUserID
    ...
    Gui, Add, Edit, x125 y134 w255 h15 vprivateServerLink -Theme cBlack BackgroundFFFFFF, %savedServerLink%

    IniRead, SavedKeybind, %settingsFile%, Main, UINavigationKeybind, \
    if (SavedKeybind = "") {
        SavedKeybind := "\"
    }
    Gui, Add, Edit, x169 y209 w20 h16 vSavedKeybind gUpdateKeybind -Theme +E0x200 +E0x800 cBlack Background2E2E2E, %savedKeybind%
    
    Gui, Font, s8 cBlack, Segoe UI
    IniRead, SavedSpeed, %settingsFile%, Main, MacroSpeed, Stable
    Gui, Add, DropDownList, vSavedSpeed gUpdateSpeed x115 y182 w50 , Stable|Fast|Ultra|Max
    GuiControl, ChooseString, SavedSpeed, %SavedSpeed%
    
    
    ; Keyboard Layout Dropdown (inside Settings group)
    IniRead, selectedLayout, %settingsFile%, Main, KeyboardLayout
    layoutOptions := "QWERTY|AZERTY|Dvorak|Colemak|Workman"

    ; If selectedLayout is empty, set a default value
    if (selectedLayout = "")
        selectedLayout := "QWERTY"

    ; Find the correct index to pre-select
    selectedIndex := 1
    Loop, Parse, layoutOptions, |
    {
        if (A_LoopField = selectedLayout) {
    ...
    Gui, Font, s8 cBlack, Segoe UI
    Gui, Add, DropDownList, x131 y326 w55 vKeyboardLayout -E0x200 Choose%selectedIndex%, %layoutOptions%
...
Gui2:
    Gui, 2:Destroy
    Gui, 2:-MinimizeBox -Caption
    Gui, 2:Font, s9 cWhite, Segoe UI
    Gui, 2:Color, 0x202020
    Gui, 2:Show, x200 y200 w520 h425
    WinSet, TransColor, 0x202020, ahk_id 2


    Gui, 2:Add, Picture, x0 y0 w520 h425 BackgroundTrans, % mainDir "rbx window background .PNG"
    Gui, 2:Add, Picture, x0 y0 w520 h425 BackgroundTrans, % mainDir "background minimize button.PNG"
    Gui, 2:Add, Picture, x0 y0 w520 h425 BackgroundTrans, % mainDir "background close button.PNG"
    Gui, 2:Add, Picture, x0 y0 w520 h425 BackgroundTrans, % mainDir "background help tab tab.PNG" 


    
    Gui, 2:Add, Text, x496 y0 w24 h24 BackgroundTrans gCloseApp2
    Gui, 2:Add, Text, x0 y0 w24 h24 BackgroundTrans gMinimizeApp
    Gui, 2:Add, Text, x0 y0 w472 h15 BackgroundTrans gDragWindow
        
		Gui, 2:Add, Tab,  x0 y24 w540 h24 , Help Window     |  How to set up |Common issues| Manual Alignment | multi instance
	
    Gui, 2:Tab, 1
    Gui, 2:Add, Picture, x0 y0 w520 h425 BackgroundTrans, % mainDir "background help tab background.PNG" 
    Gui, 2:Font, s9 cWhite norm, Segoe UI
    Gui, 2:Add, Text, x20 y90 w469 h300 BackgroundTrans,
...
Also please be sure to have the latest image file the UI is made using those so if you dont have the latest there may be some errors or the UI breaking especially when a new event releases as we have to remake the tab for it 
...
Do not enable UI navigation fix if you dont have have the ui navigation bug where the ui navigation dont start on the setting if it start anywhere else enable it 
)
		Gui, 2: Tab, 3
        Gui, 2:Add, Picture, x0 y0 w520 h425 BackgroundTrans, % mainDir "background help tab background.PNG" 
		Gui, 2: Font, s9 cWhite norm, Segoe UI
...
The ui navigation key doesnt start in the setting(top right)

Enable the ui navigation fix in the settings

The character auto allignment is is off 

Disable  auto allignt and follow the guide in the "manual allignment  tab"
)
		Gui, 2: Tab, 4
        Gui, 2:Add, Picture, x0 y0 w520 h425 BackgroundTrans, % mainDir "background help tab background.PNG" 
		Gui, 2: Font, s9 cWhite norm, Segoe UI
        Gui, 2:Add, Text, x120 y75 w469 h300 BackgroundTrans, do not do this if the auto align is working properly
		Gui, 2:Add, Text, x20 y90 w469 h300 BackgroundTrans,
(
...
Gui, 2:Add, Picture, x35 y155 w450 h250, % mainDir "camalign.png"
...
Gui3:
    Gui, 3:Show, x169 y0 w626 h1000
    Gui, 3:+Resize +MinimizeBox +SysMenu 
    Gui, 3:Margin, 10, 10
    Gui, 3:Color, c244f29
    Gui, 3:Add, Picture, x-7 y13 w724 h986, % mainDir "pets.png"

    Gui, 3:Add, Checkbox, cYellow x530 y3 gSwitch, Always on top
Switch:
	if(Always:=!Always)
		Gui,-AlwaysOnTop
	else
...
GetKey(key) {
    ...
    } else if (KeyboardLayout = "Workman") {
        if (key = "w") {
            return "d"
        } else if (key = "a") {
            return "a"
...
ClearSaves:
    ...
    IniWrite, %KeyboardLayout%, %settingsFile%, Main, KeyboardLayout
...
SaveKeyboardLayout:
    Gui, Submit, NoHide
    ; Save the selected keyboard layout
    IniWrite, %KeyboardLayout%, %settingsFile%, Main, KeyboardLayout
Return
